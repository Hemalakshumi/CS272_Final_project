...
TRAIN_LOG_PATH = os.path.join(BASE_DIR, "logs", "highway_training_rewards.npy")
TEST_REWARDS_PATH = os.path.join(BASE_DIR, "logs", "highway_test_rewards_optimized.txt")
PLOTS_DIR = os.path.join(BASE_DIR, "plots", "highway_optimized")
...
def plot_learning_curve():
    ...
    plt.title("Highway-v0 LiDAR – PPO Training Curve (Raw Reward)")
    plt.legend()
    plt.grid(True, alpha=0.3)

    out_path = os.path.join(PLOTS_DIR, "highway_optimized_learning_curve.png")
...
def plot_test_violin():
    ...
    plt.xticks([1], ["Highway PPO (LiDAR)"])
    plt.ylabel("Episodic return")
    plt.title("Highway-v0 LiDAR – PPO Test Performance")
    plt.grid(True, axis="y", alpha=0.3)

    out_path = os.path.join(PLOTS_DIR, "highway_optimized_test_violin.png")
    plt.savefig(out_path, bbox_inches="tight")
    plt.show()
    plt.close()
