...
class ParallelParkingEnv(AbstractEnv):
    ...
    def compute_reward(self, achieved_goal, desired_goal, info=None):
        ...
        # If single goal (1D), reshape to 2D for consistent handling
        single = False
        ...
        reward = np.zeros(achieved_goal.shape[0], dtype=np.float32)

        # Collision penalty (assume same vehicle for all goals)
        if getattr(self.vehicle, "crashed", False):
            reward[:] = self.config["collision_reward"]
            return reward[0] if single else reward

        # Distance between achieved and desired goals
        distances = np.linalg.norm(achieved_goal[:, :2] - desired_goal[:, :2], axis=1)

        # Heading difference
        heading_diff = self._compute_heading_diff()  # scalar, could be extended for batch

        # Base distance reward
        reward -= 0.3 * np.clip(distances / 30.0, 0, 1)

        # Out-of-bounds penalty
        x, y = achieved_goal[:, 0], achieved_goal[:, 1]
        reward += np.where((x < 0) | (x > 24) | (y < 0) | (y > 10),
                           self.config["out_of_bounds_penalty"],
                           0.0)

        # Near goal rewards
        near_goal = distances < self.config["goal_tolerance"]
        reward += near_goal * (1 - heading_diff / np.pi) * self.config["near_goal_position_reward"]

        reward += near_goal.astype(float) * float(heading_diff < np.deg2rad(5)) * self.config["additional_alignment_reward"]

        reward +=  near_goal.astype(float) * float(heading_diff < np.deg2rad(5))  * self.config["success_reward"]

        # Extra reward for being near target parking area
        in_target_area = (x >= 14) & (x <= 18) & (y >= 6) & (y <= 10)
        reward += in_target_area * self.config["near_goal_reward"]
